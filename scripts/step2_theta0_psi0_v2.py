...
# --------------------- Flux Function --------------------------
@njit
def unit_flux(r, dr, b, alpha, cos2psi, psi0, kappa_ff, kappa_K, r_in, theta0, dphi, M, r0_vac):
    r = r + (dr / 2)
...
@njit
def compute_absorption(r, dr, b, dphi, chi, M, r0_vac):
    r = r + (dr / 2)
    # Nonsingular black hole metric
    B = B_func(r, M, r0_vac)
    sqrt_B = sqrt(B) if B > 0 else 0.0
    sqrt_arg = 1.0 - (B * b**2) / r**2
    if sqrt_arg <= 0.0:
...
def process_b_group(b_value, group_df, r_in, dalpha, theta0, psi0, kappa_ff, kappa_K, dphi, opt_regime, chi, M, r0_vac):
    ...
    if opt_regime == "thin":
       ...
       for alpha, data in records.items():
        ...
        # Enable the following code when calculating optically intermediate
        for alpha, data in records.items():
...
def process_b_range(min_b, max_b, input_file, temp_file, r_in, dalpha, theta0, psi0, kappa_ff, kappa_K, dphi, step_b, r_max, opt_regime, chi, M, r0_vac):
    ...
    for b_val, group in chunk.groupby("b"):
        results.extend(process_b_group(b_val, group, r_in, dalpha, theta0, psi0, kappa_ff, kappa_K, dphi, opt_regime, chi, M, r0_vac))
...
def main():
    ...
    print(f"Step two completed in {time.time() - start_time:.2f} seconds.")
